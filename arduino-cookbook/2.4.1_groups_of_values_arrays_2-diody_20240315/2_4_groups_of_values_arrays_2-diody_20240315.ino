...
* My modification
* Turn on two LEDs in parallel: this from the button and the neighbouring one.
...
*/
 ...
 void loop(){
  for (int index = 0; index < 4; index++)
  {
    int idx_next = (index + 1) % 4;
    int val = digitalRead(inputPins[index]); // read input value
    if (val == LOW)                          // check if the switch is pressed
    {
      digitalWrite(ledPins[index], HIGH);   // LED on if the switch is pressed
      digitalWrite(ledPins[idx_next], HIGH);   // LED on if the switch is pressed
    }
    else {
